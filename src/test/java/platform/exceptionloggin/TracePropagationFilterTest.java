...
class TracePropagationFilterTest {
    ...
    void reusesIncomingTraceThroughRequestAndResponseThenClearsIt() throws Exception {
        properties.setAcceptIncomingTraceIds(true);
    ...
    @Test
    void rejectsIncomingTraceByDefaultAtTheTrustBoundary() throws Exception {
        MockHttpServletRequest request = new MockHttpServletRequest();
        request.addHeader("X-Trace-Id", "untrusted-trace-123");
        MockHttpServletResponse response = new MockHttpServletResponse();
    ...
    private TracePropagationFilter filter() {
        return new TracePropagationFilter(context, properties);
    }
}
